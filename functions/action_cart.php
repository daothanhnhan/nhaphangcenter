...
class action_cart extends library{
       ...
       {  
            ...
            if($_POST["action"] == "add"){  
              ...
              {  
                ...
                {  
                  if($_SESSION["shopping_cart"][$keys]['product_link'] == $_POST["product_link"])  
                  {  
                    $is_available++;  
            ...
            {  
                ...
                {  
                  $order_table .= '  
                    <tr>  
                      <td><img src="'.$values["product_img"].'" width="50" /></td>
  ...
  /*-------------------------------------------------*/
  public function addCart($total_price, $session_id, $name_buyer, $email_buyer, $phone_buyer, $address_buyer, $note_buyer, $user_id){

    global $conn_vn;
    $date_cart = $this->sw_get_current_weekday();
    $sql = "INSERT into cart (session_id, total_price, date_cart, name_buyer, mail_buyer, phone_buyer, address_buyer, note_buyer, user_id) values ('".$session_id."',"."'".$total_price."',"."'".$date_cart."',"."'".$name_buyer."',"."'".$email_buyer."',"."'".$phone_buyer."',"."'".$address_buyer."',"."'".$note_buyer."',"."'".$user_id."')";
    $result = mysqli_query($conn_vn,$sql);  
    return $conn_vn->insert_id;
  }
  /*-------------------------------------------------*/
  public function addItemCart($id_product, $quantity, $price, $session_id, $color, $size, $id_color, $id_size, $id_cart, $product_name, $product_link, $product_img, $user_id, $product_size, $product_color){
    global $conn_vn;
    if (isset($_SESSION['shopping_cart']) && $_SESSION['shopping_cart'] != "") {
        $totalPrice = $quantity * $price;
        $sql = "INSERT into cartdetail (id_product, session_id, price_product, qyt_product, totalprice_product, color, size, id_color, id_size, id_cart, product_name, product_link, product_img, user_id, product_size, product_color) values ('".$id_product."',"."'".$session_id."',"."'".$price."',"."'".$quantity."',"."'".$totalPrice."',"."'".$color."',"."'".$size."',"."'".$id_color."',"."'".$id_size."',"."'".$id_cart."',"."'".$product_name."',"."'".$product_link."',"."'".$product_img."',"."'".$user_id."',"."'".$product_size."',"."'".$product_color."')";
        $result = mysqli_query($conn_vn,$sql);    
        return $conn_vn->insert_id;
    }
  }
  ...
  public function getCart(){
    ...
    if(isset($_SESSION['shopping_cart']) && $_SESSION['shopping_cart'] != ""){
        foreach($_SESSION['shopping_cart'] as $key => $value){
            ...
            $line->product_name = $value['product_name'];
            $line->product_link = $value['product_link'];
            $line->product_img = $value['product_img'];
            $line->product_size = $value['product_size'];
            $line->product_color = $value['product_color'];
            $tmp = $tmp + $line->total;
            $cartArray[] = $line;            
        }
    }
  ...
  public function payment1 () {
      ...
      $session_id = session_id();    
      $id_cart = $this->addCart(0, $session_id, $name_buyer, $email_buyer, $phone_buyer, $address_buyer, $note_buyer, $user_id);
      foreach ($rows as $row) {
          $this->addItemCart($row->id_product, $row->count, $row->price, $session_id, $row->color, $row->size, $row->id_color, $row->id_size, $id_cart, $row->product_name, $row->product_link, $row->product_img, $user_id, $row->product_size, $row->product_color);
          $totalPrice += $row->total;              
      }
      if($this->updateCartTable($totalPrice,$id_cart)){
          $this->emptyCart();
  ...
  public function getCartDetail_userid ($id) {
    global $conn_vn;
    $sql = "SELECT * FROM cartdetail WHERE user_id = $id";
    $result = mysqli_query($conn_vn, $sql);
    $rows = array();
