<?php 
    ...
    function editQuotes ($id) {
        ...
        if (isset($_POST['edit_quotes'])) {
            ...
            $sql = "UPDATE quotes SET ship = $ship, total = $vnd, note = '$note' WHERE cart_id = $id";
            $result = mysqli_query($conn_vn, $sql) or die('loi1:' . mysqli_error($conn_vn));
    ...
    editQuotes($_GET['id_cart']);
    $quotes = getQuotes($_GET['id_cart']);
...
<form action="" method="post" id="addAccount1">
    ...
    <div class="rowNodeContentPage">
        ...
        <div class="boxNodeContentPage">
            ...
            <input type="text" id="ship" class="txtNCP1" name="ship" value="<?= number_format($quotes['ship']) ?>" onkeyup="money(this)" required/>
            ...
            <input type="text" id="vnd" class="txtNCP1" name="vnd" value="<?= number_format($quotes['total']) ?>" onkeyup="money(this)" required/>
            <input type="hidden" name="user_id" value="<?= $rowOrder['user_id'] ?>">
            <p class="titleRightNCP">Ghi chú</p>
            <textarea class="longtxtNCP2" name="note" ><?= $quotes['note'] ?></textarea>
            
        </div>
    </div><!--end rowNodeContentPage-->
    
    <button class="btn btnSave" type="submit" name="edit_quotes">Lưu</button>
</form>
